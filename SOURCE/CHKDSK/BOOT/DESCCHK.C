...
/* Standard definition for 2.88 MB Disketten (ED) */
...
int DescriptorCheck(RDWRHandle handle)
{
    ...
    switch (boot.descriptor)
    {
       ...
       case 0xF0:
	   /* Checks for 1.44 MB first, if this fails, checks for 2.88 MB */
	   /* Updated by Fritz and Google AI */
	    if (!CheckKnownFormats(&boot, &ValuesF0h))
	       return CheckKnownFormats(&boot, &ValuesF0h_ED);
	    else
	       return TRUE;
    }
